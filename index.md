---
layout: default
title: Home
---

<section class="hero">
  <div class="hero-grid">
    <img class="avatar" src="{{ '/assets/img/profile/avatar.png' | relative_url }}" alt="Foto de perfil de Lucía Ferreño">

    <div>
      <h1>Lucía Ferreño</h1>

      <p class="subtitle">
        Data Analyst orientada a BI y analítica aplicada.
      </p>

      <p>
        Un giro profesional enfocado en transformar datos en decisiones: reporting accionable, métricas consistentes y modelos de datos robustos.
      </p>

      <div class="cta">
        <a class="btn btn--primary" href="{{ '/cv/' | relative_url }}">Ver CV</a>
        <a class="btn" href="{{ '/projects/' | relative_url }}">Ver proyectos</a>
        <a class="btn" href="https://www.linkedin.com/in/lucia-ferreno-data-analyst" target="_blank" rel="noopener">LinkedIn</a>
      </div>

      <p style="margin-top:14px;">
        <a href="{{ '/about/' | relative_url }}">About</a> ·
        <a href="{{ '/projects/' | relative_url }}">Proyectos</a> ·
        <a href="{{ '/cv/' | relative_url }}">CV</a> ·
        <a href="{{ '/contact/' | relative_url }}">Contacto</a>
      </p>
    </div>
  </div>
</section>
